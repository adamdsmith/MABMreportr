  ...
  if (nrow(na.omit(spp_calls)) < 1) {
    ...
    p2 <- p + 
      ggplot2::geom_point(data = spp_calls, 
                          ggplot2::aes(x = lon, y = lat, fill = spp_cn), shape = 21,
                          size = 2) +
      ggplot2::scale_fill_manual("Species", values = spp_cols,
                                 guide = ggplot2::guide_legend(title.hjust =0.5,
                                                               title.position = "top",
                                                               nrow = ceiling(n_spp / 3),
                                                               byrow = TRUE)) +
      theme(legend.background = element_rect(color = "black", fill = "gray85"))
    spp_leg <- cowplot::get_legend(p2)
    
    # Construct final plot
    p <- p +
      ggplot2::geom_point(data = spp_calls, 
                          ggplot2::aes(x = lon, y = lat, fill = spp_cn, 
                                       shape = surv_date), size = 2) +
      ggplot2::scale_shape_manual("Survey date",
                                  values = poss_shapes[1:length(unique(spp_calls$surv_date))],
                                  guide = "none") +
      ggplot2::scale_fill_manual("Species", values = spp_cols, guide = "none")
    
    p <- cowplot::ggdraw(p) + cowplot::draw_grob(leg_shape, 0, 0.96, 1, 0.02)
    
    spp_ht <- 0.1 + (0.05 * (ceiling(n_spp / 3) - 1))
