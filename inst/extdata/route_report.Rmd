...
```{r multiyear_check-{{route}}}
# Check if multiple years available
n_yrs <- dplyr::n_distinct(call_info$year)
```

## Species detection summary (`r ifelse(n_yrs == 1, params$year, paste(params$stn_start_yr, params$year, sep = " - "))`)
...
n_NA <- nrow(spp_calls[is.na(spp_calls$lat) | is.na(spp_calls$lon), ])
...
if (nrow(na.omit(spp_calls)) < 1) {
  ...
  req_pckg <- c("cowplot", "devtools", "gridExtra", "rgdal", "sp", "stats")
  ...
  route_shp <- rgdal::readOGR(shp_path, tools::file_path_sans_ext(route_shp), 
                              verbose = FALSE)
  route_df <- as(route_shp, "data.frame") 
  shp_coords <- sp::coordinates(route_shp) %>% as.data.frame()
  ll_summary <- apply(rbind(bat_coords, 
                            stats::setNames(shp_coords, names(bat_coords))), 2, range)
  ll_diffs <- apply(ll_summary, 2, diff)
  ll_means <- apply(ll_summary, 2, mean)
  
  zoomlon <- ceiling(log2(360 * 2/ll_diffs[1]))
  zoomlat <- ceiling(log2(180 * 2/ll_diffs[2]))
  ...
  suppressWarnings(
    p <- ggmap::ggmap(bm, extent = "device", darken = 0.05) +
      ggplot2::geom_point(data = route_df, ggplot2::aes(x = coords.x1, y = coords.x2),
                          size = 0.5)
  )
  ...
  # Deprecated panel.margin warning suppressed
  suppressWarnings(p <- cowplot::ggdraw(p) + cowplot::draw_grob(leg_shape, 0, 0.96, 1, 0.04))
