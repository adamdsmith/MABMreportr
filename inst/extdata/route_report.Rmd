...
if (!has_data) cat(paste0("No data reported for this route in ", params$year, "."))
```
...
if (nrow(na.omit(spp_calls)) < 1) {
  ...
  route_shp <- rgdal::readOGR(shp_path, tools::file_path_sans_ext(route_shp), 
                              verbose = FALSE)
  shp_coords <- sp::coordinates(route_shp)[[1]] %>% do.call("rbind", .) %>% as.data.frame()
  ...
  suppressWarnings(
    p <- ggmap::ggmap(bm, extent = "device", darken = 0.05) +
      ggplot2::geom_point(data = shp_coords, ggplot2::aes(x = V1, y = V2), size = 0.5)
  )
